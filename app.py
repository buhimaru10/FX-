...
# ================================
# 右側スペースを広げ、説明→チャートの順で表示（横長）
# ================================
# 左は説明の簡易欄、右はチャートをできるだけ横に広く
left_narrow, right_wide = st.columns([1, 4])
...
- 手数料：建て・決済 各 {int(FEE_PER_LOT_PER_SIDE)} 円/枚（固定）
        """
    )
    ...
    """
    # コンポーネントの高さもチャートに合わせる
    components.html(tradingview_embed, height=640)

with left_narrow:
    # 必要ならここに補助説明や注意書き等（右の横幅を優先するため簡素に）
    st.markdown("")
